...
async function AddUser () {
    ...
    const userRef = doc(db,"user",data.value.username)
  ...
  // console.log("lol" + userSnap.data().firstname)
    ...
    if(userSnap.exists()) { 
        ...
        await setDoc(doc(db, "user", data.value.username), {
    password:data.value.password ,
   username: data.value.username,
    createOn: Timestamp.fromDate(new Date())
});

await setDoc(doc(db, "cart", data.value.username), {
    count:0 ,
   price: 0,
    user: data.value.username
});
        alert("สร้าง user เรียบร้อย");
    }
...
<div>
	...
	<div class="bg-red-200 w-full sm:w-3/4 max-w-lg p-12 pb-6 shadow-2xl rounded">

		<div v-if="!formShow" class="text-center p-20 text-2xl"> login success</div>
		<div v-if="formShow">
		<div class="text-red-500 pb-4 text-3xl font-semibold">Pizza SignUp</div>
		<input
